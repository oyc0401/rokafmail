...
export async function mailApi({
  ...
  // 유저가 아니면 400에러 ㄱㄱ
  if (!user) {
    return { message: "해당 유저를 찾을 수 없습니다.", status: 404 };
  }
  ...
  if (contents.length > 1200) {
    return { message: "내용은 1200자를 넘을 수 없습니다.", status: 400 };
  }
  if (title.length > 300) {
    return { message: "제목은 300자를 넘을 수 없습니다.", status: 400 };
  }
  if (name.length > 100) {
    return { message: "이름은 100자를 넘을 수 없습니다.", status: 400 };
  }
  if (relationship.length > 100) {
    return { message: "관계는 100자를 넘을 수 없습니다.", status: 400 };
  }
  ...
  return { message: "편지 전송 성공!", status: 200 };
}
