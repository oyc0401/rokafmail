...
import { insertUser, insertUserQueue, updateUserMember } from "src/db";
// 로그인을 하면 먼저 DB에 저장한다.
...
async function checkUser({ id, name, birth, generation }) {
  ...
  if (connect) {
    ...
    await updateUserMember({ id, memberSeq, sodae, connect: true });
  } else {
    console.log("유저 인증 실패, 인증 큐에 저장하는 중...");
    await insertUserQueue({ userId: id });
  }
